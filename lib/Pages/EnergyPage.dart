...
import 'package:ecoapp/Models/energyModel.dart';
...
class _EnergyScreenState extends State<EnergyScreen> {
  final controllers =
  List.generate(getEnergyQuestions().length, (index) => TextEditingController());
  List<EnergyQuestion> energyquestionList = getEnergyQuestions();
  List<String?> selectedAnswers = List.generate(getEnergyQuestions().length, (index) => null);
  List<bool> isEditing = List.generate(getEnergyQuestions().length, (index) => false);
            ...
            children: [
              ...
              Expanded(
                child: ListView.builder(
                  itemCount: energyquestionList.length,
  ...
  Widget buildQuestionItem(int index) {
    final question = energyquestionList[index];
  ...
  void saveAnswers() {
    for (int i = 0; i < energyquestionList.length; i++) {
      if (isEditing[i]) {
        saveAnswer(i);
      }
    }
  ...
  void submitSurvey() async {
    final answers = energyquestionList.asMap().map((index, question) {
      return MapEntry(
        index.toString(),
        {
    ...
    await FirebaseFirestore.instance.collection("EnergyAnswers").add({
      "answers": answers,
    });
